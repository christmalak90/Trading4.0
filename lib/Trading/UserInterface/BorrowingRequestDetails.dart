...
class BorrowingRequestDetails extends StatefulWidget {
  final String transactionId;

  const BorrowingRequestDetails({Key? key, required this.transactionId}) : super(key: key);

  @override
  _BorrowingRequestDetailsState createState() => _BorrowingRequestDetailsState();
}

class _BorrowingRequestDetailsState extends State<BorrowingRequestDetails> {
  ...
  bool isLoading = true;
  bool bidAlreadyPlaced = false; //Used to know if the user has already placed a bid on the request
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        appBar: buildAppBar("REQUEST DETAILS", Colors.blue.shade200, Colors.white),
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  if (transaction.userId == FirebaseAuth.instance.currentUser!.uid) {
                    displayToastMessage("Cannot place bid on your own borrowing request", "failed", context);
                  } else if (bidAlreadyPlaced) {
                    displayToastMessage("You have already placed a bid on this request, you can go to the activity page to edit your bid.", "failed", context);
                  } else {
                        ...
                        children: transaction.bidders.map((bidder) {
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: [
                                BoxShadow(
                                  color: (bidder.userId == FirebaseAuth.instance.currentUser!.uid) ? Colors.blue : Colors.grey,
                                  spreadRadius: 0,
                                  blurRadius: 0,
                                  offset: Offset(0, 0),
                                ),
                            ...
                            child: Row(
                              children: [
                                Container(
                                  decoration: BoxDecoration(
                                    color: (bidder.userId == FirebaseAuth.instance.currentUser!.uid) ? Colors.blue.shade200 : Colors.grey.shade200,
                                    border: Border.all(color: (bidder.userId == FirebaseAuth.instance.currentUser!.uid) ? Colors.blue.shade300 : Colors.grey.shade300),
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        "Rate(%)",
                                        style: TextStyle(
                                          color: (bidder.userId == FirebaseAuth.instance.currentUser!.uid) ? Colors.blue.shade700 : Colors.grey.shade700,
                                          fontSize: 11,
                                        ),
                                      ),
                                      Text(
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        "Lending Bid",
                                        style: TextStyle(color: (bidder.userId == FirebaseAuth.instance.currentUser!.uid) ? Colors.blue : Colors.grey, fontSize: 10),
                                      ),
                                      Row(
                                        children: [
                                          Text(
  ...
  Future<void> getBorrowingRequestDetails() async {
