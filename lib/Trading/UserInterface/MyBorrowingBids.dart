...
import 'package:user_app/Trading/UserInterface/MyBorrowingBidDetails.dart';

class MyBorrowingBids extends StatefulWidget {
  static const String idScreen = "myBorrowingBids";

  const MyBorrowingBids({Key? key}) : super(key: key);

  @override
  _MyBorrowingBidsState createState() => _MyBorrowingBidsState();
}

class _MyBorrowingBidsState extends State<MyBorrowingBids> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        appBar: buildAppBar("MY BORROWING BIDS", Colors.grey, Colors.white),
    ...
    return Container(
      ...
      child: Column(
        children: [
          SizedBox(height: 10),
          Text(
            "This page shows all the bids that you have placed to borrow money.",
                          ...
                          transaction.bidders.forEach((bidder) {
                            if (bidder.userId == FirebaseAuth.instance.currentUser!.uid) {
                              widget = GestureDetector(
                                onTap: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (context) => MyBorrowingBidDetails(transactionId: transaction.transactionId)));
                                },
                                child: Container(
  ...
  Future<void> getMyBorrowingBids() async {
    await businessLogic.viewMyBorrowingBids().then(
      (String? value) {
        if (value == null) {
          setState(() {
            transactions = [];
